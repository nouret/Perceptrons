...
def mr():
    return (-1 + numpy.random.random() * 2) / 1

def counterror(Per1, Per2, Per3):
    ErRr = 0
...
def count(Per1, Per2, Per3, N):
    ...
    minper2 = Per2
    minper3 = Per3
    minErRrP, minErRrM = counterror(Per1, Per2, Per3)
    tmp = 0
    while True:
        ...
        tmp += 1
        if tmp % 100 == 0:
            ErRrP, ErRrM = counterror(Per1, Per2, Per3)
            if minErRrP ** 2 + minErRrM ** 2 > ErRrP ** 2 + ErRrM ** 2:
                minErRrP = ErRrP
                minErRrM = ErRrM
                minper1 = Per1
                minper2 = Per2
    ...
    return minErRrP, minErRrM, minper1, minper2, minper3
